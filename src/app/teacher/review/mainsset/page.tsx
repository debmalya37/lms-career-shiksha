...
// Expects APIs:
// GET  /api/teacher/mainsset                => lists mains sets + submissions
// POST /api/teacher/mainsset/review/[id]    => { userId, marks: { [questionIndex]: number } }
// ===================================================================
...
export default function TeacherMainsReviewPage() {
  const [sets, setSets] = useState<MainsWithSubs[]>([]);
  const [loading, setLoading] = useState(false);
  const [marksMap, setMarksMap] = useState<Record<string, Record<number, number>>>({}); // mainsId_userId -> { qIdx: mark }
  const [saving, setSaving] = useState<Record<string, boolean>>({});

  useEffect(() => { fetchForTeacher(); }, []);

  async function fetchForTeacher() {
    try {
      setLoading(true);
      const res = await fetch('/api/teacher/mainsset');
  ...
  function keyFor(mainsId: string, userId: string) { return `${mainsId}_${userId}`; }
        ...
        {sets.map((s) => (
          <Card key={s._id}>
            ...
            <CardContent className="grid gap-4">
              {s.submissions.length === 0 && <p className="text-sm text-gray-500">No submissions yet.</p>}

              {s.submissions.map((sub) => (
                <div key={sub.user._id} className="p-3 border rounded">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <div className="font-medium">{sub.user.name || sub.user.email || sub.user._id}</div>
                      <div className="text-sm text-gray-500">Status: {sub.status}</div>
                    </div>
                    <div>
                      <div className="text-sm text-gray-600">Total: {sub.totalMarks ?? '-'} </div>
                    </div>
                  </div>

                  <div className="grid gap-3">
                    {sub.answers.map((ans, i) => (
                      <div key={i} className="grid sm:grid-cols-3 gap-2 items-center p-2 border rounded">
                        <div className="sm:col-span-2">
                          <div className="font-medium">Q{ans.questionId+1}</div>
                          <p>{}</p>
                          <a href={ans.answerFile} target="_blank" rel="noreferrer" className="text-sm text-blue-600 underline">Open answer</a>
                        </div>
                        <div className="sm:col-span-1">
                          <Input type="number" placeholder="Marks" value={(marksMap[`${s._id}_${sub.user._id}`]?.[ans.questionId] ?? ans.marks ?? '') as any} onChange={(e) => setMark(s._id, sub.user._id, ans.questionId, Number(e.target.value))} />
                        </div>
                      </div>
                    ))}

                    <div className="flex justify-end">
                      <Button onClick={() => submitReview(s._id, sub.user._id)} disabled={!!saving[`${s._id}_${sub.user._id}`]}>
