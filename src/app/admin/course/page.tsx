...
const ManageCourses = () => {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      try {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/api/subjects`);
  ...
  useEffect(() => {
    if (subject) {
      const fetchTopics = async () => {
        try {
          const res = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/api/topics?subject=${subject}`);
          setTopics(res.data);
        } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/course`, courseData);
      setTitle('');
      setDescription('');
      setSubject('');
      setTopic('');
  ...
  const handleAddSubject = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/newSubject`, { name: newSubjectName });
  ...
  const handleAddTopic = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/topics`, { name: newTopicName, subject });
